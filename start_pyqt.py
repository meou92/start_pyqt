...
def show_battery():
    ...
    """
    Label(win,[0,0,80,27],text="Battery",style=s%color2).show()
    label_battery = Label(win,[80, 0, 100, 27],text="",style=s%color2)
    label_battery.setAlignment(align.AlignRight)
    label_battery.show()
    scale_battery = QtWidgets.QProgressBar(win)
    scale_battery.setRange(0,100)
    scale_battery.setStyleSheet(progress_style % color2)
